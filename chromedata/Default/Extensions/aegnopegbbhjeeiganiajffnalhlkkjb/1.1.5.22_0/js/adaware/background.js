    ...
    chrome.runtime.onMessage.addListener(function listener(request, sender, sendResponse) {
        switch (request.what) {
            ...
            case 'requestPermission':
                    ...
                    if (request.name == "popup") {
                        chrome.storage.local.get(['permissionDate'], function (data) {
                            if (data.permissionDate !== undefined) {
                                var result = Math.abs(data.permissionDate - Date.now()) / 1000;
                                var days = Math.floor(result / 86400);
                                var minutes = Math.floor(result / 60) % 60;
                                if (days >= 7) { // show every 7 days if "no thanks" is clicked
                                    chrome.storage.local.set({openPanelCount: 1}, function () {});
                                    chrome.storage.local.set({permissionDate: Date.now()}, function () {});
                                    chrome.storage.local.set({openPanelCount: 3}, function () {});
                                    sendResponse({showPermissionPopup: true});
                                }
                            } else {
                                chrome.storage.local.set({permissionDate: Date.now()}, function () {});
                            }
            ...
            case 'refreshCurrentPage':
                ...
                break;
            case 'getParentWindowSize':
                    chrome.tabs.sendMessage(sender.tab.id, { text: "getWindowSize" }, function (response) {
